...
public class XlsDataSetWriter {
	...
	/**
	 * A special format pattern used to create a custom {@link HSSFDataFormat}
	 * which marks {@link Date} values that are stored via POI to an XLS file.
	 * Note that it might produce problems if a normal numeric value uses this
	 * format pattern incidentally.
	 */
	...
	private static final Logger logger = LoggerFactory
			.getLogger(XlsDataSetWriter.class);
	 ...
	 */
	public void write(IDataSet dataSet, OutputStream out) throws IOException,
			DataSetException {
				...
				for (int k = 0; k < columns.length; k++) {
					...
					if (value != null) {
						...
						if (value instanceof Date) {
							...
							cell.setCellValue(new HSSFRichTextString(DataType
									.asString(value)));
						}
					}
				}
	...
	protected void setNumericCell(HSSFCell cell, BigDecimal value,
		...
		if (logger.isDebugEnabled())
			logger.debug(
					"setNumericCell(cell={}, value={}, workbook={}) - start",
		...
		if (logger.isDebugEnabled())
			logger.debug("Using format '{}' for value '{}'.",
					String.valueOf(format), value);
	...
	// // cal.setTimeInMillis(date.getTime() % (24*60*60*1000));
	// cal.set(Calendar.SECOND, (int) (date.getTime() % (24*60*60*1000)) /
	// (1000) );
	// cal.set(Calendar.MINUTE, (int) (date.getTime() % (24*60*60*1000)) /
	// (1000*60) );
	// cal.set(Calendar.HOUR, (int) (date.getTime() % (24*60*60*1000)) /
	// (1000*60*60) );
	// // cal.set(1900, 0, 1); // 1.1.1900
