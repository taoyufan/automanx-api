...
/**
 * <p>
 * <code>TestExecutionListener</code> which provides support for executing tests
 * within transactions by using
 * {@link org.springframework.transaction.annotation.Transactional
 * @Transactional} and {@link NotTransactional @NotTransactional} annotations.
 * </p>
 * <p>
 ...
 * database during a test run with &#064;NotTransactional will
 * <strong>not</strong> be run within a transaction. Similarly, test methods
 * that are not annotated with either &#064;Transactional (at the class or
 * method level) or &#064;NotTransactional will not be run within a transaction.
 * </p>
 * <p>
 ...
 * certain <em>set up</em> or <em>tear down</em> code outside of a transaction.
 * <code>TransactionalTestExecutionListener</code> provides such support for
 ...
 */
public class TransactionalListener extends AbstractTestListener {

	private static final Log logger = LogFactory
			.getLog(TransactionalListener.class);
	...
	public void beforeTestMethod(TestContext testContext) throws Exception {
		...
		TransactionAttribute transactionAttribute = this.attributeSource
				.getTransactionAttribute(testMethod, testContext.getTestClass());
		...
		if (transactionDefinition != null) {
			if (logger.isDebugEnabled()) {
				logger.debug("Explicit transaction definition ["
						+ transactionDefinition + "] found for test context ["
						+ testContext + "]");
			}
			TransactionContext txContext = new TransactionContext(
					getTransactionManager(testContext), transactionDefinition);
	...
	protected void runAfterTransactionMethods(TestContext testContext)
		...
		if (afterTransactionException != null) {
			rethrowException(afterTransactionException);
		}
	}
	...
	private void startNewTransaction(TestContext testContext,
		...
		if (logger.isInfoEnabled()) {
			logger.info("Began transaction (" + this.transactionsStarted
					+ "): transaction manager [" + txContext.transactionManager
					+ "]; rollback [" + isRollback(testContext) + "]");
		}
	}
	...
	private void endTransaction(TestContext testContext,
		...
		if (logger.isTraceEnabled()) {
			logger.trace("Ending transaction for test context [" + testContext
					+ "]; transaction manager [" + txContext.transactionStatus
					+ "]; rollback [" + rollback + "]");
		}
	...
	protected final PlatformTransactionManager getTransactionManager(
		...
		if (this.configAttributes == null) {
			this.configAttributes = retrieveTransactionConfigurationAttributes(testContext
					.getTestClass());
		}
		String transactionManagerName = this.configAttributes
				.getTransactionManagerName();
		try {
			return (PlatformTransactionManager) SpringContextManager
					.getApplicationContext().getBean(transactionManagerName,
	...
	protected final boolean isDefaultRollback(TestContext testContext)
			throws Exception {
		return retrieveTransactionConfigurationAttributes(
				testContext.getTestClass()).isDefaultRollback();
	}
	 ...
	 */
	private TransactionConfigurationAttributes retrieveTransactionConfigurationAttributes(
			Class<?> clazz) {
		...
		if (config != null) {
			...
			transactionManagerName = (String) getDefaultValue(annotationType,
					"transactionManager");
			defaultRollback = (Boolean) getDefaultValue(annotationType,
					"defaultRollback");
		}
	...
	private static PlatformTransactionManager getTransactionManager(
		...
		if (beanFactory instanceof ConfigurableListableBeanFactory) {
			...
			return getTransactionManager(
					(ConfigurableListableBeanFactory) beanFactory, qualifier);
		} else if (beanFactory.containsBean(qualifier)) {
			// Fallback: PlatformTransactionManager at least found by bean name.
			return (PlatformTransactionManager) beanFactory.getBean(qualifier,
					PlatformTransactionManager.class);
		} else {
			throw new IllegalStateException(
					"No matching PlatformTransactionManager bean found for bean name '"
							+ qualifier
							+ "'! (Note: Qualifier matching not supported because given BeanFactory does not "
